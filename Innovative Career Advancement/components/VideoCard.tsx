...
export default function VideoCard({
  ...
  const glow = tone === "primary" ? "shadow-[0_0_50px_rgba(255,30,60,0.4)]" : "shadow-[0_0_50px_rgba(255,30,60,0.22)]";
  const ring = "group-hover:border-red";
    ...
    >
      <motion.div
        className={`absolute inset-0 bg-gradient-to-br ${
          tone === "primary" ? "from-red/70 via-ink to-red/10" : "from-red/35 via-ink to-red/10"
        }`}
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-ink via-ink/70 to-transparent p-5 pt-14">
        <span className={`text-xs font-bold uppercase tracking-[0.2em] ${tone === "primary" ? "text-red" : "text-red/70"}`}>
          {category}
        </span>
